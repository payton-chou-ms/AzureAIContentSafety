...
"""
測試 genai_example.json - Prompt Shields 檢測 V4
...
V4 新增功能:
- 新增 Indirect Attack 測試 (將 Human Prompt 放入 documents)
...
def comprehensive_analysis(
    ...
    start_time = time.time()
    shield_jailbreak_result = analyze_prompt_shield_jailbreak(endpoint, subscription_key, human_prompt)
    ...
    start_time = time.time()
    shield_indirect_result = analyze_prompt_shield_indirect(endpoint, subscription_key, human_prompt)
    ...
    # 5. GPT-5-nano 語意分析 (同時檢查 human_prompt 和 system_prompt)
    start_time = time.time()
...
def test_genai_dataset(
    ...
    """
    測試 genai_example.json 資料集 - 使用綜合分析 V4
    """
    print("=" * 80)
    print("開始測試 genai_example.json - 綜合安全分析 V4")
